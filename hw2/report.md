...
程式，讀取圖檔和輸出圖檔分別利用 2 個 SC_THREAD 去執行，不像 part1 的程式是共用 1 個 SC_THREAD，是因為在 GauFilter.cpp 中，  
卷積資料並分別寫入 result_r,result_g,result_b ，卻一直沒有進行讀取，造成 sc_fifo 存放太多筆資料，產生 sc_fifo 的堵塞導致程式卡死  
，後來才分別利用 2 個 SC_THREAD 去執行。
